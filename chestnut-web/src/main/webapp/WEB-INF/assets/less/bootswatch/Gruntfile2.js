module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    clean: {
      build: {
        src: ['*/build.less', '*/build.scss', '!global/build.less', '!global/build.scss','*/build.js']
      }
    },
    concat: {
      options: {
  ...
  grunt.registerTask('build', 'build a regular theme', function(theme, compress) {
    ...
    var concatLess = {src: 'global/build.less', dest:  theme + '/build.less'};
    var concatjs = {src: ['bower_components/bootstrap/dist/js/bootstrap.js',theme + '/bootswatch.js'], dest: theme + '/build.js'};
    grunt.config('concat.less', concatLess);
    grunt.config('concat.js', concatjs);
    
    var lessSrc = [ theme + '/' + 'build.less' ];
    var lessDest = '<%=builddir%>/' + theme + '/bootstrap.css';
    ...
    grunt.task.run(['concat:less','concat:js', 'less:dist', 'prefix:' + lessDest, 'clean:build',
      compress ? 'compress:'+lessDest+':'+'<%=builddir%>/' + theme + '/bootstrap.min.css':'none',
      compress ? 'compress:'+concatjs.src+':'+'<%=builddir%>/' + theme + '/bootstrap.min.js':'none'
    		  ]);
  });
