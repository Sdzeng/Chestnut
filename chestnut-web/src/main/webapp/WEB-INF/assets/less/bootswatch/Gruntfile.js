module.exports = function (grunt) {
  ...
  grunt.registerTask('build', 'build a regular theme', function(theme, compress) {
    ...
    var fileBaseName= grunt.config.get('baseName');
    
    var concatLess = {src: 'global/build.less', dest:  theme + '/build.less'};
    var concatjs = {src: 'bower_components/bootstrap/dist/js/bootstrap.min.js', dest: theme + '/'+fileBaseName+'.min.js'};
    grunt.config('concat.dist', concatLess);
    grunt.config('concat.js', concatjs);
    
  
    var lessDest= '<%=builddir%>/' + theme + '/'+fileBaseName+'.css';
    ...
    grunt.task.run(['concat', 'less:dist', 'prefix:' + lessDest, 'clean:build',
      compress ? 'compress:'+lessDest+':'+'<%=builddir%>/' + theme + '/'+fileBaseName+'.min.css':'none']);
  });
