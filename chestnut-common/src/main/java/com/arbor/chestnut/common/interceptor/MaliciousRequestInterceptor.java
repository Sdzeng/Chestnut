package com.arbor.chestnut.common.interceptor;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import com.arbor.chestnut.common.constant.OperationCode;
import com.arbor.chestnut.common.constant.HttpCode;
...
public class MaliciousRequestInterceptor extends BaseInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		String preRequest = (String) session.getAttribute(OperationCode.PREREQUEST);
		Long preRequestTime = (Long) session.getAttribute(OperationCode.PREREQUEST_TIME);
		...
		if (preRequestTime != null && preRequest != null) { // 过滤频繁操作
			if ((url.equals(preRequest) || allRequest)
				...
				Integer maliciousRequestTimes = (Integer) session.getAttribute(OperationCode.MALICIOUS_REQUEST_TIMES);
				if (maliciousRequestTimes == null) {
					maliciousRequestTimes = 1;
				} else {
					maliciousRequestTimes++;
				}
				session.setAttribute(OperationCode.MALICIOUS_REQUEST_TIMES, maliciousRequestTimes);
				...
				session.setAttribute(OperationCode.MALICIOUS_REQUEST_TIMES, 0);
			}
		}
		session.setAttribute(OperationCode.PREREQUEST, url);
		session.setAttribute(OperationCode.PREREQUEST_TIME, System.currentTimeMillis());
